...
const {SLACK_TOKEN, JIRA_USERNAME, JIRA_PASSWORD} = process.env;
...
module.exports = async (pluginConfig, context) => {
  const {
    ...
    env: {CI_PROJECT_URL, CI_PROJECT_NAME, CI_PIPELINE_URL},
  ...
  for (const issueId of issueIds) {
    ...
    await jira.addComment(
      issueId,
      `Released in [${gitTag}](${tagUrl}). (Pipeline ${CI_PIPELINE_IID})[${CI_PIPELINE_URL}].`
    );
  ...
  const slackText = `Tickets Published ${
    ticketIds.length > 0 ? ticketIds.join(', ') : 'None'
  }`;
